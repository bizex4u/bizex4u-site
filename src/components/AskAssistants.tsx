 ...
 * this page and /llms.txt. The visitor starts a session that has to
 * fetch the site — which is the AEO/GEO move. There is no on-site chat.
 *
 * Gemini has no reliable chat-URL prefill, so that icon opens Google
 * AI Mode (Gemini with citations). The other three take `q=`.
 */
...
export default function AskAssistants({
  ...
  const q = encodeURIComponent(askPrompt(pageUrl));
  const muted = tone === "plum" ? "text-on-plum-dim" : "text-on-sand-dim";
  const icon = tone === "plum" ? "text-on-plum/80 hover:text-on-plum" : "text-on-sand/80 hover:text-on-sand";

  return (
    <div
      data-ask-assistants={variant}
      className={
        variant === "page"
          ? `mt-10 flex flex-nowrap items-center gap-x-3 border-t pt-5 ${
              tone === "plum" ? "border-rule-plum" : "border-rule-sand"
            }`
          : "flex flex-nowrap items-center gap-x-2"
      }
    >
      <p className={`eyebrow shrink-0 ${muted}`}>
        {variant === "page" ? "Ask this page" : "Ask"}
      </p>
      <ul
        className="m-0 flex list-none items-center p-0"
        aria-label="Ask an assistant about this page"
      >
        {assistants.map((a) => {
          const Icon = a.icon;
          return (
            <li key={a.id}>
              <a
                ...
                title={`Ask ${a.name}`}
                className={`inline-flex h-10 w-10 items-center justify-center rounded-sm transition-colors duration-150 ${icon}`}
              >
                <Icon />
              </a>
            </li>
